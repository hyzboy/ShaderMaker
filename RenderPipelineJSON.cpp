...
namespace
{
    ...
    FROM_JSON_BEGIN(VkPipelineRasterizationStateCreateInfo)
        FROM_JSON_BOOL(depthClampEnable)
        FROM_JSON_BOOL(rasterizerDiscardEnable)
        FROM_JSON_ENUM(VkPolygonMode,polygonMode)
        FROM_JSON_ENUM(VkCullModeFlags,cullMode)
        FROM_JSON_ENUM(VkFrontFace,frontFace)
        FROM_JSON_BOOL(depthBiasEnable)
        FROM_JSON_FLOAT(depthBiasConstantFactor)
        FROM_JSON_FLOAT(depthBiasClamp)
        FROM_JSON_FLOAT(depthBiasSlopeFactor)
        FROM_JSON_FLOAT(lineWidth)
    FROM_JSON_END
    ...
    FROM_JSON_BEGIN(VkPipelineMultisampleStateCreateInfo)
        FROM_JSON_ENUM(VkSampleCountFlagBits,rasterizationSamples)
        FROM_JSON_BOOL(sampleShadingEnable)
        FROM_JSON_FLOAT(minSampleShading)
        FROM_JSON_BOOL(alphaToCoverageEnable)
        FROM_JSON_BOOL(alphaToOneEnable)
    FROM_JSON_END
    ...
    FROM_JSON_BEGIN(VkPipelineInputAssemblyStateCreateInfo)
        FROM_JSON_ENUM(VkPrimitiveTopology,topology)
        FROM_JSON_BOOL(primitiveRestartEnable)
    FROM_JSON_END
    ...
    FROM_JSON_BEGIN(VkGraphicsPipelineCreateInfo)
        FROM_JSON_OBJECT(pDepthStencilState)
        FROM_JSON_OBJECT(pColorBlendState)
        FROM_JSON_OBJECT(pRasterizationState)
        FROM_JSON_OBJECT(pMultisampleState)
        FROM_JSON_OBJECT(pInputAssemblyState)
    FROM_JSON_END
...
void SaveToJSON(const OSString &filename,const VkGraphicsPipelineCreateInfo *state)
{
    ...
    SaveJson(root,filename,error_info);
}

bool LoadFromJSON(const OSString &filename,VkGraphicsPipelineCreateInfo *state)
{
    Json::Value root;
    OSString error_info;

    if(!LoadJson(root,filename,error_info))
        return(false);
